...
def get_arm_angles_transformed(angles):
    ...
    result['j_ankle2_l'] = angles['j_ankle2_l']-math.pi/9
    result['j_ankle2_r'] = angles['j_ankle2_r']-math.pi/9
    result['j_thigh1_l'] = angles['j_thigh1_l']-math.pi/9
    result['j_thigh1_r'] = angles['j_thigh1_r']-math.pi/9
 #   result['j_ankle1_r'] = angles['j_ankle1_r']+math.pi/8
  #  result['j_ankle1_l'] = angles['j_ankle1_l']+math.pi/8

#   result['j_pan'] = angles['j_pan']+math.pi/8


#    result['j_tibia_l'] = angles['j_tibia_l']-math.pi/9
 #   result['j_thigh2_l'] = angles['j_thigh2_l']+math.pi/2
  #  result['j_ankle1_l'] = angles['j_ankle1_l']+math.pi/4
    return result

def sit_down_transformed(angles):
	result = {}
	result['j_tibia_l'] = angles['j_tibia_l']-math.pi/4
	result['j_tibia_r'] = angles['j_tibia_r']-math.pi/4
	return result
...
def reset_arms(darwin):
    ...
    action6 = {
	'j_ankle1_l':math.radians(5),
	'j_ankle2_l':math.radians(-35),
	'j_thigh2_l':math.radians(55)
    }
    action7 = {
	'j_thigh1_l':math.radians(30),
        'j_thigh1_r':math.radians(30),
	'j_ankle2_r':math.radians(20),
	'j_ankle2_l':math.radians(12)
    }

    darwin.set_angles_slow(get_arm_angles_transformed(action1),1)
    time.sleep(2)
    act = {
	'j_tibia_l':math.radians(-100)
    }
    darwin.set_angles_slow(act,1)
    time.sleep(1)
    darwin.set_angles_slow(raise_leg_transformed(action2),1)
    time.sleep(3)
    darwin.set_angles_slow(action3,1)
    time.sleep(3)
    act3 = {
	'j_thigh2_l':math.radians(35),
        'j_tibia_l':math.radians(-40),
	'j_ankle1_l':math.radians(25),
        'j_ankle2_l':math.radians(-25)
    }
    darwin.set_angles_slow(act3,1) 

    act4 = {
	'j_thigh1_r':math.radians(0),
	'j_thigh1_l':math.radians(0)
    }

    act5 = {
	'j_thigh2_l':math.radians(30),
	'j_thigh2_r':math.radians(30),
	'j_ankle2_l':math.radians(20)
    }

    time.sleep(1)
    darwin.set_angles_slow(act5,1)

 #  darwin.set_angles_slow(turn_body_transformed(action4),1)
    time.sleep(1)
  #  darwin.set_angles_slow(push_body_transformed(action5),1)
    time.sleep(2)
 #   darwin.set_angles_slow(action6,1)
    time.sleep(2)
#    darwin.set_angles_slow(action7,1)


def raise_leg_transformed(angles):
	result = {}
	result['j_thigh1_l'] = angles['j_thigh1_l']-math.pi/6
#	result['j_tibia_l'] = angles['j_tibia_l']-math.pi/2
	result['j_thigh2_l'] = angles['j_thigh2_l']+math.pi/1.90
	result['j_thigh2_r'] = angles['j_thigh2_r']
	result['j_ankle1_l'] = angles['j_ankle1_l']+math.pi/8
	return result

def push_leg_transformed(angles):
	result = {}
	result['j_tibia_l'] = angles['j_tibia_l']-math.pi/2
	result['j_ankle1_l'] = angles['j_ankle1_l']+math.pi/6 
	return result

def turn_body_transformed(angles):
	result = {}
	result['j_pelvis_r'] = angles['j_pelvis_r']-math.pi/12
	return result

def push_body_transformed(angles):
	result = {}
	result['j_thigh2_r'] = angles['j_thigh2_r']-math.pi/10
	result['j_ankle1_l'] = angles['j_ankle1_l']+math.pi/3.2
	result['j_tibia_l'] = angles['j_tibia_l']-math.pi/3
	result['j_thigh1_l'] = angles['j_thigh1_l']-math.pi/5
#	result['j_thigh2_l'] = angles['j_thigh2_l']+math.pi/16
	return result
...
def initialize(): 
    ...
    while True:
        ...
        if inp == ';':
            ...
            reset_arms(darwin)
        elif inp.find('joint') == 0:
            parts = inp.split()
            action = {}
            action[parts[1]] = math.radians(float(parts[2]))
            darwin.set_angles_slow(action,1)
